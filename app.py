...
with st.container():
    ...
    st.subheader("🌼 ข้อมูลอนิเมะ 🌼")

    # ช่องกรอกข้อมูลผู้ใช้
    genre_count = st.slider("จำนวนประเภทของอนิเมะ (Genre Count)", 1, 10, 3)
    episodes = st.slider("จำนวนตอนของเรื่อง", 1, 200, 12)
    studio_score = st.slider("คะแนนความน่าเชื่อถือของสตูดิโอ (0-10)", 0.0, 10.0, 7.5)
    release_year = st.slider("ปีที่ออกฉาย", 1980, 2025, 2015)
    
    is_sequel = st.selectbox("มีภาคต่อหรือไม่", ["💥 ไม่มี (No)", "🌺 มี (Yes)"])
...
if st.button("🍎 Predict Rating (ทำนายคะแนน)"):
    ...
    try:
        ...
        st.success(f"คะแนนที่คาดการณ์: **{prediction:.2f} / 10**")
...
# import streamlit as st # ใช้สร้างเว็บแอป Python
# import pandas as pd
# import pickle # ใช้โหลดโมเดล Machine Learning ที่เคยบันทึกไว้ (model.pkl)

# # โหลดโมเดลที่ฝึกแล้ว + Scaler
# model, scaler = pickle.load(open("model.pkl", "rb"))

# # ส่วนตกแต่ง CSS (สีพื้นหลัง + ปุ่ม + กล่อง)
# page_bg = """
# <style>
# html, body, [class*="css"] {
...
# # หัวข้อเว็บ
# st.title("🎌 Anime Rating Prediction Web Application 🎌")
# st.write("🩷 ทำนายความนิยมของอนิเมะ 🩷")

# # Input Form สำหรับรับข้อมูลจากผู้ใช้
# with st.container():
...
#     is_sequel_num = 1 if is_sequel == "มี" else 0
